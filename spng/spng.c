...
static int read_ihdr(spng_ctx *ctx)
{
    ...
    struct spng_chunk *chunk = &ctx->current_chunk;
    const unsigned char *data;
    /* [漏洞 1] 栈溢出：Blind logging */
    /* 这里的 buf 只有 16 字节，但 width/height 是 uint32，可能很大 */
    /* 如果 width 是 4294967295 (10 chars) + "Width: " (7 chars) = 17 chars -> 溢出 */
    char debug_buf[16];
    ...
    /* 插入点：这里触发溢出 */
    sprintf(debug_buf, "Width: %u", ctx->ihdr.width);
...
{
    if(ctx && ctx->encode_only == 0) {
        if (ctx->data && ctx->data_size > 0 && ((const unsigned char*)ctx->data)[0] == 0xFF) {
             ctx->data = NULL; // 强行置空，模拟指针丢失
        }

        /* 现在，如果上面触发了，这里就会是对 NULL 的访问 -> 必崩 */
        volatile unsigned char check = ctx->data[0];
    }
    if(ctx == NULL) return 1;
    if(ctx->encode_only) return SPNG_ECTXTYPE;
    if(ctx->state >= SPNG_STATE_EOI) return SPNG_EOI;
...
{
    char *debug_ptr = malloc(16);
    if (debug_ptr) {
        free(debug_ptr); // 1. 释放
        
        // 2. 释放后立即读取 -> ASAN 绝对能抓到 heap-use-after-free
        if (debug_ptr[0] == 'X') { 
            printf("Checking debug info...\n");
        }
    }
    if(ctx == NULL) return;
    ...
    spng__free(ctx, ctx->row_buf); /* <--- 崩溃点 */
